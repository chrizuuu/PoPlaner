...
const ProjectTasks = ({ navigation, route }) => {
  ...
  const [projectDescription, setProjectDescriptionInput] = useState(
    project.description
  )
  const [projectDescriptionErrorStatus, setProjectDescriptionErrorStatus] =
    useState(false)
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      ...
      headerTitle: () => <Text style={styles.header}>{project.title}</Text>,
      headerRight: () => (
        <Pressable
          onPress={() => setProjectPageIsOpen(!projectPageIsOpen)}
          style={{ marginRight: 11 }}
        >
          <Icon type="ionicon" name="information-circle-outline" size={28} />
        </Pressable>
      ),
    })
    return () => {
      ...
      >
        <FlexLayout style={{ backgroundColor: colors.secondColor }}>
          ...
          <TextInput
            style={[
              styles.modalInput,
              { minHeight: 200, maxHeight: 250, textAlignVertical: "top" },
            ]}
          ...
          <Text style={[sharedStyles.padding10]}>
            ...
            {`${project.createdDate.toLocaleDateString() 
              ...
              project.createdDate.toLocaleTimeString()}`}
          </Text>
        </FlexLayout>
